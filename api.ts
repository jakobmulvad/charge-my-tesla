...
import { ITeslaApiResponse, ITeslaVehicle, ITeslaChargeState, ITeslaVehicleData, ITeslaApiToken } from './types';
...
const api = async <T>(method:Method, url: string) : Promise<T> => {
  ...
  const response = await Axios.request<ITeslaApiResponse<T>>({ url, method, baseURL: 'https://owner-api.teslamotors.com/api/1/vehicles',  headers: { Authorization: 'Bearer ' + cachedToken.access_token }});
  return response.data.response;
}

export const getVehicleList = async (): Promise<ITeslaVehicle[]> => {
  return api<ITeslaVehicle[]>('get', '/');
}

export const getVehicle = async (vehicleId: string): Promise<ITeslaVehicle> => {
  return api<ITeslaVehicle>('get', `/${vehicleId}`);
}
...
export const getVehicleData = async (vehicleId: string): Promise<ITeslaVehicleData> => {
  return api<ITeslaVehicleData>('get', `/${vehicleId}/vehicle_data`);
}
